 ...
 * @returns 
 */
function ChampionDetail({ user, login })
{
    ...
    const [flagFavored, setFlagFavored] = useState(false);
    ...
    useEffect(() =>
    {
        if (!flagLoaded) {
            ...
            {
                axios
                    .get(url + `/champion/complete/info?id=${id}`)
                    .then((res) =>
                    {
            ...
            {
                axios
                    .get(url + `/champion/counter?id=${id}`)
                    .then((res) =>
                    {
        ...
        if (!flaged) {
            async function delFavor()
            {
                let i = 0
                for (i; i < user.favourite.length; i += 1) {
                    if (user.favourite[i] == id) {
                        setFlagFavored(true)
                        break
                    }
                }
                if (i === user.favourite.length) {
                    setFlagFavored(false)
                }
                setFlaged(true)
            }
            delFavor()
    ...
    }, [id, flaged, setFlaged, flagFavored, setFlagFavored, details, setDetails, flagLoaded, setFlagLoaded]);
    ...
    const buttonHandler = () =>
    {
        ...
        if (!flagFavored) {
            // Add Favorite
            axios
                .put(url + `/user/favourite?user_id=${user.userID}&champion_id=${id}`)
                .then((res) =>
                {
                    ...
                    axios
                        .get(url + `/user/info?id=${user.userID}`)
                        .then((res) =>
                        {
            ...
            axios
                .delete(url + `/user/favourite?user_id=${user.userID}&champion_id=${id}`)
                .then((res) =>
                {
                    if (res.data.hasOwnProperty('Error_message')) {
                        alertError(res.data['Error_message']);
            ...
            <div className="champion-details-container">
                ...
                <div className="champion-right-column">
                    ...
                    <div className="champion-button">
                        {user.userID !== '-1' ?
                            (<button className='button-add-favourite' onClick={buttonHandler}>{flagFavored ? 'Remove Favourite' : 'Add To Favourite'}</button>)
                            :
                            (<></>)
                        }
                    </div>
